...
Data and files are passed from browser to python using multipart POST requests. The same can be returned back to the browser.

## Python modules 
1. main.py -- serves as main. It shouldn't be changed.
2. webserv.py -- holds the code related to the webserver operation. It shouldn't be changed.
3. post.py -- parses multipart post requests and returns methos _POST() and _FILES() alike PHP $_POST[] and $_FILE[]. These are dictionaries, where _POST() provides dictionary of key (the variable name provided in post request) and value. _FILES() provides dictionary with keys, like the former, and values are a list of file names and files. It shouldn't be changed.
4. common.py -- holds functions and keywords common to other files in the project. It should not be changed.
...
- Browser connects python http server on free port between 50000 and 59999. When the browser is closed, it sends a last request "close". The Server receives it and quits python.
